...
########################################
# Started July 2012
# Update August 2016		: remove possibility to compare (diff) stdout with testfile.res
# Adapted November 2017 : adapted to project ASM MIPS (comparaison of obj files. Then dropped next year.)
# Update November 2018
# Update August 2020 - N Castagne : correct a bug with sed on Mac OS X when extracting TEST_RETURN_CODE from test file. See NC aout 2020.
# Update August 2020 - N Castagne : correct help and various typos.
########################################
# No restriction on usage nor dissemination
########################################
# Help : see readme file     and ./simpleUnitTest.sh
# Bugs and questions : nicolas.Castagne@grenoble-inp.fr , francois.cayre@grenoble-inp.fr
########################################
...
printHelp()
{
	...
	echo ""
	echo "Each test testfile in <args_testfiles> should contain, in a comment:"
	echo "	# TEST_RETURN_CODE=X"
	echo "         where X is in {PASS,FAIL,SKIP}"
	echo "And optionally a :"
	echo "  # TEST_COMMENT=string gives information about the test"
	echo
	echo " The <executablefile> will be run on each testfile and :"
	echo "	1/ generate a file mytestfile.out containing the output (stdout) of the program "
	echo "	2/ compare the return value of the program with the expected return code value (PASS, ie 0 was expected, or FAIL, ie non-0 was expected)"
# NC aout 2020 : possibility to compare (diff) stdout with testfile.res was droped from simpleUnitTest in 2016 or 2017
#	echo "	3/ compare the awaited output (mytestfile.res) with the obtained output (mytestfile.out)"
	echo "	3/ display result and generate a report"
	echo ""
	echo " Required arguments are :"
	echo "	simpleUnitTest -e <executablefile> <args_testfiles>"
	echo " Where :"
	...
	echo "	-q				: quiet mode : less output while testing"
	echo "	-v 				: verbose outputs (echoes the test file and the out file)"
	echo "	-c				: cleans the testing files before running :"
	echo "							removes <summaryfile>"
	echo "							removes any test output eg testfile.out"
	echo "	-o <summaryFile>		: if used, writes the tests' results into summaryfile"
...
# - the test file itself containing the expected exit code, and an optional comment
...
# then comparing it to the expected value (as specified in comment in the test file)
#
...
{	if [ $# -ne 3 ]
	...
	then
		...
		DO_NOT_CHECK)
			...
			;;
		*)
			echoOutputHighlight "Expected return code $exe_expected_result (in .info test file) is not a valid token... Cannot check return code."
...
#	There should be comments indicating TEST_RETURN_CODE and TEST_COMMENT
#       in each test file
...
# NC aout 2020 : possibility to compare (diff) stdout with testfile.res was droped from simpleUnitTest in 2016 or 2017,
# so the following does not apply anymore
...
executeTest()
{
	...
	# NC aout 2020
	# Le sed qui suit ne marche pas sur MacOS. Remplacement par version de
